...
private:
    ...
    /*로봇과 아머거리 구하기*/
    double assign_distance(short x, short y);
public:
...
double data_control::assign_distance(short x, short y) {
	double distance = 0;
...
void data_control::assign_robot(short num, bbox_t info) {
	...
	double x_dist = (info.x + info.w*0.5)*0.5;
	this->dataset.angle_hori[index] = std::atan2(x_dist,sqrt(pow(this->dataset.distance[index],2.0)-pow(x_dist,2.0)));}
