...
int Blackboard::GetLockedOnEnemy() const {
    return locked_on_enemy_;
}

void Blackboard::SetLockedOnEnemy(int index) {
    if(index != 1 && index != 2){
        ROS_INFO("blackboard set enenmy lockon error: wrong index. index should be 1 or 2");
    }
    locked_on_enemy_ = index;
}

int Blackboard::GetEnemyPriority() const {
    return enemy_priority_;
}

void Blackboard::SetEnemyPriority(int index) {
    if(index != 1 && index != 2){
        ROS_INFO("blackboard set enenmy priority error: wrong index. index should be 1 or 2");
    }
    enemy_priority_ = index;
}
