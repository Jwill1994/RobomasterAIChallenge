...
private:
    ...
    float SEARCH_VEL = 0.032;  // SEARCH_VEL * SEARCH_COUNT = total angle (1.5~1.7)
...
void GimbalControl::calc_offset(float dist) {
    ...
    x_offset = 0.5*dist - 16.5;
    y_offset = (int) (30 - 18*dist + 1.56*dist*dist);
    if(dist < 0.9) y_offset = 50;
    //printf("y offset: %d\n", y_offset);
}
...
//        printf("current: %d\n", this->current_time);
	if(this->prev_time == 0) {
        ...
        //printf("dt: %d\n", dt);
        if(pixel_error_x < X_CENTER_PIXEL * 0.4 && pixel_error_x > -X_CENTER_PIXEL * 0.4) this->yaw_i_error += pixel_error_x * dt;
        if(pixel_error_y < Y_CENTER_PIXEL * 0.4 && pixel_error_y > -Y_CENTER_PIXEL * 0.4) this->pitch_i_error += pixel_error_y * dt;
