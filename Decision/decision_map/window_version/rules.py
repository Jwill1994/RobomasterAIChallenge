...
class rules():    
    def __init__(self, size, team):
        ...
        self.pm.rectangular_assign(0, 8.000, 0, 5.000, 50)
    ...
    def enemy_overlap(self, e1, e2, value):
        ...
        print(p2p, bisector)        
        self.pm.sector_assign(e1[0], e1[1], 0, p2p[0]/math.cos(p*bisector)/2, p2p[1], 2*p*bisector, -value/constant)
        self.pm.sector_assign(e2[0], e2[1], 0, p2p[0]/math.cos(p*bisector)/2, p2p[2], 2*p*bisector, -value/constant)
        
    def reload_zone(self, stance, game_time, ammo_left, bullets, value):
        our = [4.000,0.500,0.100]
        enemy = [4.000,4.500,0.500]
        if self.team == 'red':
