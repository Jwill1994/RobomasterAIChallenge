...
//        ros::Time GetTimeGameStarted() const;
        const int GetTimePassedFromGameStart();
        GameState GetGameState();
        //Zone Info
        bool GetIsBuffZoneOnline();
        int GetTimeLastBuffed();
        const int GetTimeLeftForBuffZoneToOnline();
        bool GetIsReloadZoneOnline();
        int GetTimeLastReloaded();
        const int GetTimeLeftForReloadZoneToOnline();  //TODO need to be changed to abide rule
        //HP Info
        int GetMyHealth();
        ...
        int GetTimeLastSeen(PlayerType who) const;
...
//        ros::Time time_game_started_;
        int time_passed_from_game_start_;
        ...
        int time_last_buffed_;
        int time_left_for_buff_zone_to_online_;
        bool is_reload_zone_online_=true;
        int time_last_reloaded_ = 0;
        int time_left_for_reload_zone_to_online_;
    ...
    private: // Perception Info
        ...
        int time_enemy_1_last_seen_;
        int time_enemy_2_last_seen_;
        int time_enemy_last_seen_;

        //Ally Pose
        geometry_msgs::PoseStamped ally_pose_from_vision_;  //ally pose from vision
        int time_ally_last_seen_;
