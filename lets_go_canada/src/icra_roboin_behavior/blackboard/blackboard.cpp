...
//}
const int Blackboard::GetTimePassedFromGameStart() {
    UpdateTime();
...
int Blackboard::GetTimeLastBuffed() {
    if(robotstatus.id==3 || robotstatus.id==4){
...
const int Blackboard::GetTimeLeftForBuffZoneToOnline() {
    UpdateTime();
...
const int Blackboard::GetTimeBuffLeft() {
    UpdateTime();
...
int Blackboard::GetTimeLastSeen(PlayerType who) const{
    switch(who){
...
void Blackboard::UpdateTime(){
    time_passed_from_game_start_ = 180 - static_cast<int>(gamestatus.remaining_time);
    if(is_buff_zone_online_){
        time_left_for_buff_zone_to_online_ = 0;
    } else {
        time_left_for_buff_zone_to_online_ = (time_last_buffed_ + 30) - this->GetTimePassedFromGameStart(); //buffzone opens every 1min
        if(time_left_for_buff_zone_to_online_ <= 0){
            time_left_for_buff_zone_to_online_ = 0;
        }
    }
    time_buff_left_ = (time_last_buffed_ + 30) - this->GetTimePassedFromGameStart();
    if(time_buff_left_ <= 0){
        time_buff_left_ = 0;
    }
    if(supplierstatus.status!=0){
        time_left_for_reload_zone_to_online_ = 0;
    }
    else if(supplierstatus.status == 0){
        if(time_last_reloaded_<=60){
            time_left_for_reload_zone_to_online_ = 60 - time_last_reloaded_;
        }
        else if(60<time_last_reloaded_<=120){
            time_left_for_reload_zone_to_online_ = 120-time_last_reloaded_;
        }
    }
    }
