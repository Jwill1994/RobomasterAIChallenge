...
class rules():    
    def __init__(self, size, team, number):
        ...
        self.team = team
        self.robot_num = number        
        self.inner_wall_tuple = ((1.200,2.200, 3.750,4.000), (1.400,1.650,1.400,2.400), (3.250,3.500,0,1.000),(3.500,4.500,2.375,2.625),(5.800,6.800,1.000,1.250),(6.350,6.600,2.600,3.600),(4.500,4.750,4.000,5.000))
        
        self.stance = 'aggressive'
        self.health = 2000
        self.ammo = 40
        self.pos = [0, 0]   
        self.pos_set = deque([], 50)  
    ...
    def init(self):
        ...
        self.pos_set.appendleft(np.round(self.pos, 2))
    ...
    def reload_zone(self, stance, team_pos, reload_count, bullets, value):
        ...
        #   value = value * 2        
         ...
         #   self.pm.circle_assign_gradient(team_pos[0], team_pos[1], distance*constant, 2*value/constant, constant*2)
          #  self.pm.circle_assign_gradient(team_pos[0], team_pos[1], distance, -2*value/constant, constant)
        
        self.pm.circle_assign_gradient(our[0], our[1], enemy[2]*4, value/5, 10)
        #else:
        if reload_count > 0 and bullets < 10:
            self.pm.square_assign(our[0], our[1], our[2], value)      
        
    def move_cost(self, stance, value):
        self.pm.circle_assign_gradient(self.pos[0], self.pos[1], 10, -value, 10)
        
        
    def wall_cover(self, stance, enemy_pos, value):
        gamma = math.pi/2
        ...
        for item in self.inner_wall_tuple:           
            ...
            for item_sub in target:          
                ...
                a2 = self.pm.p2pRelation(item_sub, self.pos)[0]
                b1 = self.pm.p2pRelation(minimum, enemy_pos)[0]
                b2 = self.pm.p2pRelation(minimum, self.pos)[0]
                temp = 4*a1*a1 + a2*a2
                dist = 4*b1*b1 + b2*b2
                if temp < dist:
                    minimum = item_sub
        ...
        r2 = r1[0] + 1
        constant = max(self.pm.p2pRelation(self.pos, enemy_pos)[0]/2,1)            
    ...
    def pos_view(self):
        self.pm.square_assign(self.pos[0], self.pos[1], self.robot_radius, -10, mode='abs')
    ...
    def current_zone(self, n, value):
        ...
        try:
            ...
            self.pm.circle_assign(self.pos[0], self.pos[1], self.robot_radius*2, -value_scaled)
            stuck = list(itertools.islice(self.pos_set, 0, 40))
            if np.sum(np.var(stuck, axis=0)) < 0.01:
                self.pm.circle_assign(self.pos[0], self.pos[1], self.robot_radius*2, 0, mode='abs')
            return value_scaled
        except:
            pass
        
    def stuck_recovery(self, n, value):
        last = min(n, 25)        
        angle = math.pi*random.random()
        dist = 5 * random.random()

        target = list(itertools.islice(self.pos_set, 0, last))
        if np.sum(np.var(target, axis=0)) < 0.00001 and len(target)> n-1:        
            self.pm.random_assign(150)                     
            self.pm.sector_assign(self.pos[0], self.pos[1], self.robot_radius, self.robot_radius*dist, angle, math.pi/6, value, mode='abs')
            self.pm.sector_assign(self.pos[0], self.pos[1], self.robot_radius, self.robot_radius*dist, angle+math.pi, math.pi/6, value='abs')                  
